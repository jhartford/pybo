...
def run_ep_iteration(mu, Sigma, ymax, sn2):
    ...
    # we will assume that Sigma is invertible. We'll later have to invert (or
    # take the cholesky) of this plus tauHat, which could initially be very
    # small, so we won't want to invert tauHat.
    R = sla.cholesky(Sigma)
    invSigma, _ = sla.lapack.dpotri(R)
    a = sla.cho_solve((R, False), mu)
    ...
    while True:
        ...
        while True:
            ...
            vals, _ = np.linalg.eig(np.diag(tauHatNew) + invSigma)
        ...
        # the new posterior.
        R = sla.cholesky(invSigma + np.diag(tauHat))
        V, _ = sla.lapack.dpotri(R)
        m = sla.cho_solve((R, False), rhoHat + a)
...
def run_ep(X, y, xstar, ell, sf2, sn2):
    ...
    K = get_cov(X, xstar, 1/ell**2, sf2, 0.0)
    ...
    # NOTE: here K is the gradient between [X, g, h] where g are the gradients
    # and h are the off-diagonal elements of the hessian.
    ...
    m, v = run_ep_iteration(mu, Sigma, max(y), sn2)
    m = np.r_[b, m]
    ...
    Kstar = get_crosscov(Xstar, X, xstar, 1/ell**2, sf2, 0)
...
def predict_ep(Xtest, X, y, xstar, ell, sf2, sn2):
    ...
    Ktest = get_crosscov(Xtest, X, xstar, 1/ell**2, sf2, 0)
